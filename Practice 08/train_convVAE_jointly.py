...
logP = np.load("./inputs/logP.npy").astype(float)
...
train_logP = logP[0:N_train]
val_molecules = molecules[N_train:N_train+N_val]
val_logP = logP[N_train:N_train+N_val]
...
for epoch in range(num_epochs):
    ...
    for i in range(num_train):
        ...
        logP_batch = train_logP[i*batch_size:(i+1)*batch_size]
        cost = model.train(mol_batch, logP_batch)

    num_val = val_molecules.shape[0]//batch_size
    for i in range(num_val):
        mol_batch = val_molecules[i*batch_size:(i+1)*batch_size]
        logP_batch = val_logP[i*batch_size:(i+1)*batch_size]
        Y, _logP, cost = model.test(mol_batch, logP_batch)
        if( i % 50 == 0):
            mae = np.mean(np.abs(logP_batch - _logP.flatten()))
            print ("test_iter : ", i, ", epoch : ", epoch, " ", cost)
            accuracy1_, accuracy2_ = accuracy(mol_batch, Y)
            print ("accuracy : ", accuracy1_, accuracy2_, "logP MAE:", mae)
            for i in range (10):
