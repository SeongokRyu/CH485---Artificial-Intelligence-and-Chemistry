...
if( len(sys.argv) == 5 ):
    ...
    hidden_dim2 = int(sys.argv[2])
    init_lr = float(sys.argv[3])
    reg_scale = float(sys.argv[4])

model_name = 'smiles_rnn_logP2_' + str(num_layer) + '_' + str(hidden_dim2) + '_' + str(init_lr) + '_' + str(reg_scale)
print (model_name)
...
def HighwayNetwork(h):
    out_dim = int(h.shape()[2])
    _h = tf.layers.dense(h, units=out_dim, activation=tf.nn.relu)
    gate = tf.layers.dense(h, units=out_dim, activation=tf.nn.sigmoid)

    output = tf.multiply(gate,_h) + tf.multiply((1.0-gate),h)
    return output
...
### RNN Layer
cell = []
cell.append(tf.nn.rnn_cell.GRUCell(hidden_dim1))
...
for t in range(epoch_size):
    ...
    for i in range(batch_train):
        ...
        L_batch = np.ones(batch_size).astype(int)*120
