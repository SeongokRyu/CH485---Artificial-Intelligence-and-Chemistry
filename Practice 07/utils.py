...
def smiles_to_onehot2(smi_list):
    def smiles_to_vector(smiles, vocab, max_length):
        while len(smiles)<max_length:
            smiles +=" "
        return [vocab.index(str(x)) for x in smiles]
    ...
    for smi in smi_list:
        smi_onehot = smiles_to_vector(smi, list(vocab), 120)
        #smi_onehot = smiles_to_vector(smi, list(vocab), len(smi))
        smi_total.append(smi_onehot)
        len_total.append(len(smi))
    return np.asarray(smi_total), np.asarray(len_total)
